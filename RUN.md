...
# クリティカルパスに乗っている。
#
# phase0 の感情タグ [平][喜][哀][怒][驚][優] を入れる。remove_unspeechable /
# remove_markdown はどちらも角括弧を除去しないが、LLM/emotion_tags.py が
# remove_unspeechable の手前でタグを剥がすので TTS には届かない。タグは
# --qwen3_tts_emotion_refs のどの参照音声でクローンするかを選ぶのに使う。
# タグが出なかった場合は [平] にフォールバックするだけで、壊れはしない。
#
...
返答の先頭に感情タグを必ず1つ置く:
[平] ふつうの会話 / [喜] うれしい・楽しい / [哀] 悲しい・しんみり
[怒] 怒り・抗議 / [驚] 驚き / [優] やさしく寄り添う
タグは先頭に1つだけ。それ以外に角括弧は使わない。
...
./.venv/bin/speech-to-speech local \
  ...
  --qwen3_tts_emotion_refs ~/voices/companion/refs.json \
  --qwen3_tts_language ja
```

`--qwen3_tts_emotion_refs` は感情タグごとの参照音声を並べた JSON。`平` は必須で、
定義されていないスロットはすべてそこに落ちる。`相槌` はモデルがタグで選ぶものでは
なく、ターン冒頭の短い反応句（「うん、」「そっか、」）に自動で割り当てられる。
`--qwen3_tts_ref_audio` / `--qwen3_tts_ref_text` は**マニフェストを指定しない**ときと、
GGML の `ref_spk` を使うときのための単一参照。マニフェストに不備（ファイル欠落、
`text` が空、未知のスロット名、`平` の未定義）があると起動時に例外で止まる。
黙って1音声に落ちることはない。マニフェストを外せば従来どおり1音声で動く。

冒頭句と本文が別の参照音声で読まれるのは `--stream_batch_sentences 1` のときだけ。
2以上だと冒頭句が本文と同じバッチに入るため、分割は無効になる（壊れはしない）。
