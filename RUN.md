...
### 処理中の合図（`--thinking-sound`）

発話が終わってから最初の音が出るまで、STT + LLM + 最初の TTS チャンクぶんの
無音が空く。TTFA が p50 で 2 秒を超える構成では、この無音が「落ちた」ように
聞こえる。同梱クライアントがそこに小さなループ音を鳴らすようにした。

鳴らす場所は `PlaybackBuffer.write()` ――応答音声を書いたあと、足りないぶんを
無音で埋めているまさにその場所に**加算**する。置き換えではなく加算なので、
応答が届いた瞬間の 80ms は合図がフェードしながら重なり、切れ目で鳴らない。

`is_active()` は応答音声だけを見たままにしてある。ここに合図を含めると
`--block-mic-during-playback` が思考中もマイクを閉じ、割り込めなくなる。

- 開始は VAD の `speech_stopped`（打鍵ターンは `TextTurnSubmitter.submit()` の末尾）
- 停止は最初の `response.output_audio.delta`。音を出さずに終わるターン用に
  `response.done` と `error` でも止める
- `speech_started` は既存の `playback.clear()` 経由で止まる
- 既定は 0.3 秒待ってから鳴らす。速いターンで一発だけ鳴るのを避けるため

音はファイルではなく生成している。C6→E6→G6 の3音上昇アルペジオを 1.4 秒周期で、
残りは無音（無音率 84%）。周期は TTFA p50 2.1 秒に対して決めた ―― 通常の待ちで
1〜2 回鳴って喋り出す長さで、ビープの連打にならない。

矩形波だが倍音をナイキストの手前で打ち切って加算しているので、エイリアスのざらつき
は出ない。各音は 4ms のアタックと 0 まで落ちる減衰包絡を持つ。両端が厳密に 0 なので、
ループの継ぎ目でもフェードの途中でもクリックしない。

音色は `thinking_sound.py` の `_PULSES`（開始秒・周波数・長さのタプル）と
`_PULSE_PERIOD_S` にまとまっている。`--thinking-sound-file` でファイル差し替えも可
（librosa で再生レートにリサンプル）。

**ゲインは環境で詰める。** 既定 `0.12` は控えめ側に振ってある。スピーカーの音は
マイクに回り込むので、上げすぎると VAD が自分の合図で発話開始と判定する。
